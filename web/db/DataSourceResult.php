<?php
// define("TABLE_SCHEMA","dgtwfm");
define('TABLE_SCHEMA','studentgroupingsystem');
class DataSourceResult {
	...
	function __construct($dsn="mysql:host=mysql;dbname=studentgroupingsystem", $username="root", $password="test1234", $driver_options=null) {
    ...
    private function total($tableName, $properties, $request) {
        ...
        if (isset($request->filter)&&isset($request->filter->filters)) {
    ...
    private function groupBy($data, $groups, $table, $request, $propertyNames) {
        if (count($groups) > 0) {
            if((strpos($groups[0]->field, '.') !== false)){
                $field = explode('.',$groups[0]->field)[1];
            }else{
    ...
    private function where($properties, $filter, $all) {
        ...
        //ในกรณีที่มีการค้นหาจาก field ที่มีการตั้ง alias 
        //if (in_array($field, $propertyNames)) {
            $type = "string";
    ...
    public function read($table, $properties, $request = null,$groupby=null) {
        $result = array();
    ...
    public function readJoin($table, $joinTable, $properties, $key, $column, $request = null) {//$column หากต้องการ select หลาย column ให้ใช้เครื่องหมาย comma คั่น
        ...
        for ($index = 0, $count = count($result['data']); $index < $count; $index++) {
            ...
            foreach($col as $item){
                $result['data'][$index][$item] = empty($data[0][$item])?null:$data[0][$item];
            }
